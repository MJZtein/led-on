function CMenuMode() {
    ...
    this._init = function() {
        ...
        _oTextTitleBack = new createjs.Text("", " 48px " + PRIMARY_FONT, SECONDARY_FONT_COLOUR);
        _oTextTitleBack.textAlign = "center";
        _oTextTitleBack.lineWidth = 600;
        _oTextTitleBack.textBaseline = "middle";
        _oTextTitleBack.x = CANVAS_WIDTH / 2 + 2;
        _oTextTitleBack.y = CANVAS_HEIGHT / 2 - 198;
        _oTextTitleBack.text = TEXT_SELECT;
        _oContainer.addChild(_oTextTitleBack);

        _oTextTitle = new createjs.Text("", " 48px " + PRIMARY_FONT, PRIMARY_FONT_COLOUR);
        _oTextTitle.textAlign = "center";
        _oTextTitle.lineWidth = 600;
        _oTextTitle.textBaseline = "middle";
        _oTextTitle.x = CANVAS_WIDTH / 2;
        _oTextTitle.y = CANVAS_HEIGHT / 2 - 200;
        _oTextTitle.text = TEXT_SELECT;
        _oContainer.addChild(_oTextTitle);

        var iButtonXPos = CANVAS_WIDTH / 2;
        var iButtonYPosEasy = CANVAS_HEIGHT / 2 - 80;
        var iButtonYPosMedium = CANVAS_HEIGHT / 2 + 50;
        var iButtonYPosHard = CANVAS_HEIGHT / 2 + 180;
        var iButtonYPosExtreme = CANVAS_HEIGHT / 2 + 310;
        var iFontSize = 36;
        var iTextOffset = 65;

        var oSpritePlayButton = s_oSpriteLibrary.getSprite('but_menu_bg');
        _oButPlayEasy = new CTextButton(iButtonXPos, iButtonYPosEasy, oSpritePlayButton, TEXT_EASY, PRIMARY_FONT, PRIMARY_FONT_COLOUR, iFontSize);
        _oButPlayEasy.setTextPosition(iTextOffset);
        _oButPlayEasy.addEventListenerWithParams(ON_MOUSE_UP, this._onButPlayRelease, this, MODE_EASY);

        _oButPlayMedium = new CTextButton(iButtonXPos, iButtonYPosMedium, oSpritePlayButton, TEXT_MEDIUM, PRIMARY_FONT, PRIMARY_FONT_COLOUR, iFontSize);
        _oButPlayMedium.setTextPosition(iTextOffset);
        _oButPlayMedium.addEventListenerWithParams(ON_MOUSE_UP, this._onButPlayRelease, this, MODE_MEDIUM);

        _oButPlayHard = new CTextButton(iButtonXPos, iButtonYPosHard, oSpritePlayButton, TEXT_HARD, PRIMARY_FONT, PRIMARY_FONT_COLOUR, iFontSize);
        _oButPlayHard.setTextPosition(iTextOffset);
        _oButPlayHard.addEventListenerWithParams(ON_MOUSE_UP, this._onButPlayRelease, this, MODE_HARD);

        _oButPlayExtreme = new CTextButton(iButtonXPos, iButtonYPosExtreme, oSpritePlayButton, TEXT_EXTREME, PRIMARY_FONT, PRIMARY_FONT_COLOUR, iFontSize);
        _oButPlayExtreme.setTextPosition(iTextOffset);
